...
const Gallery: React.FC<GalleryProps> = ({ photos, onAddPhoto, onDeletePhoto, onClearAll, userRole, onInquire }) => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        setNewPhotoData(prev => ({ ...prev, url: reader.result as string }));
        setUploadError(null);
      };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const photo: Photo = {
      ...
      settings: {
        ...
        iso: newPhotoData.iso,
        lens: newPhotoData.lens
      }
    };
  ...
  return (
    <div className="py-24 px-6 max-w-7xl mx-auto">
      ...
      {photoToDelete && (
        <div className="fixed inset-0 z-[300] bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-6">
          <div className="bg-white border border-slate-200 rounded-3xl w-full max-w-md p-8 shadow-2xl">
            <h3 className="text-2xl font-black mb-4 text-center text-slate-900">Delete this photo?</h3>
            <p className="text-slate-500 text-center mb-8 font-medium">This action cannot be undone. It will be removed from your portfolio immediately.</p>
            <div className="flex gap-4">
              <button 
                onClick={() => setPhotoToDelete(null)}
                className="flex-1 bg-slate-100 text-slate-700 font-black py-3 rounded-xl hover:bg-slate-200 transition-all uppercase tracking-widest text-xs"
              >
                Cancel
              </button>
              <button 
                onClick={handleFinalDelete}
                className="flex-1 bg-red-600 text-white font-black py-3 rounded-xl hover:bg-red-500 transition-all shadow-lg uppercase tracking-widest text-xs"
              >
                Delete
              </button>
            </div>
      ...
      {selectedPhoto && (
        <div className="fixed inset-0 z-[200] bg-sky-50/95 backdrop-blur-xl flex items-center justify-center p-4 md:p-12 overflow-y-auto">
          <button 
              ...
              <div className="flex flex-col md:flex-row gap-8">
                <div className="flex-1 space-y-4">
                  ...
                  <input 
                    ...
                    className="w-full bg-sky-50 border border-slate-200 rounded-2xl px-6 py-4 text-slate-900 font-bold focus:border-cyan-500 outline-none"
